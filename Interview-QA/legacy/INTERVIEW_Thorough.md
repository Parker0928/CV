...
**大章速览** 快速跳转；展开各章可看到小节标题（**题目编号 · 短标题**）。
...
- [题目1 · 闭包](#题目1闭包)
- [题目2 · 继承方式对比与原型链污染](#题目2继承方式对比与原型链污染)
- [题目3 · 事件循环（Event Loop）](#题目3事件循环event-loop)
- [题目4 · Promise（A+ 与手写）](#题目4promisea-与手写)
- [题目5 · 柯里化与偏函数](#题目5柯里化与偏函数)
- [题目6 · this 绑定与箭头函数](#题目6this-绑定与箭头函数)
- [题目7 · 垃圾回收与内存泄漏](#题目7垃圾回收与内存泄漏)
- [题目8 · 深拷贝与浅拷贝](#题目8深拷贝与浅拷贝)
- [题目9 · 异步编程演进](#题目9异步编程演进)
- [题目10 · 提升与块级作用域](#题目10提升与块级作用域)
...
- [题目1 · 从 URL 到页面渲染](#题目1从-url-到页面渲染)
- [题目2 · 浏览器缓存](#题目2浏览器缓存)
- [题目3 · 跨域与 CORS](#题目3跨域与-cors)
- [题目4 · 虚拟 DOM 与 diff](#题目4虚拟-dom-与-diff)
- [题目5 · 重排与重绘](#题目5重排与重绘)
- [题目6 · HTTP 各版本与 HTTPS](#题目6http-各版本与-https)
...
- [题目7 · Web 安全（XSS / CSRF 等）](#题目7web-安全xss--csrf-等)
- [一、XSS跨站脚本攻击](#一xss跨站脚本攻击)
- [二、CSRF跨站请求伪造](#二csrf跨站请求伪造)
- [三、其他常见攻击防护](#三其他常见攻击防护)
- [题目8 · 同源策略与跨页面通信](#题目8同源策略与跨页面通信)
- [题目9 · Service Worker 与 PWA](#题目9service-worker-与-pwa)
- [题目10 · 内存溢出与内存泄漏](#题目10内存溢出与内存泄漏)
...
- [Vue 方向](#vue-方向)
- [Vue · Vue2 / Vue3 与响应式](#题目1vue2--vue3-与响应式)
- [Vue · Vue 响应式原理](#题目2vue-响应式原理)
- [Vue · Vue 虚拟 DOM、diff 与指令](#题目3vue-虚拟-domdiff-与指令)
- [Vue · Vue 组件通信](#题目4vue-组件通信)
- [Vue · Vuex 与 Pinia](#题目5vuex-与-pinia)
- [Vue · 生命周期与 keep-alive](#题目6生命周期与-keep-alive)
- [Vue · 自定义指令与插件](#题目7自定义指令与插件)
- [Vue · Teleport、Suspense 与编译优化](#题目8teleportsuspense-与编译优化)
- [React 方向](#react-方向)
- [React · Fiber 架构](#题目1fiber-架构)
- [React · Hooks 原理](#题目2hooks-原理)
- [React · 类/函数组件与 diff](#题目3类函数组件与-diff)
- [React · useEffect / useLayoutEffect](#题目4useeffect--uselayouteffect)
- [React · 组件通信与 Context](#题目5组件通信与-context)
- [React · 合成事件](#题目6合成事件)
- [React · React 性能优化](#题目7react-性能优化)
- [React · React 18](#题目8react-18)
...
- [题目1 · 前端工程化体系](#题目1前端工程化体系)
- [题目2 · 构建工具对比（Webpack / Vite 等）](#题目2构建工具对比webpack--vite-等)
- [题目3 · Webpack 原理与 Loader/Plugin](#题目3webpack-原理与-loaderplugin)
- [题目4 · Tree-Shaking 与体积优化](#题目4tree-shaking-与体积优化)
- [题目5 · 模块化演进（CJS / AMD / ESM）](#题目5模块化演进cjs--amd--esm)
...
- [题目1 · 性能优化全方案（五维）](#题目1性能优化全方案五维)
- [一、网络层面优化](#一网络层面优化)
- [二、渲染层面优化](#二渲染层面优化)
- [三、代码层面优化](#三代码层面优化)
- [四、资源层面优化](#四资源层面优化)
- [五、构建层面优化](#五构建层面优化)
- [题目2 · 性能监控与核心指标](#题目2性能监控与核心指标)
...
- [题目1 · 防抖与节流](#题目1防抖与节流)
- [题目2 · instanceof 原理](#题目2instanceof-原理)
- [题目3 · 数组去重](#题目3数组去重)
- [题目4 · 数组 flat（深度）](#题目4数组-flat深度)
...
- [题目1 · 权限控制（路由 / 按钮 / 接口）](#题目1权限控制路由--按钮--接口)
- [题目2 · 前端 SSO](#题目2前端-sso)
- [题目3 · 大数据量渲染](#题目3大数据量渲染)
...
- [题目1 · 驼峰转短横线（camel → kebab）](#题目1驼峰转短横线camel--kebab)
- [题目2 · 上传、分片与断点续传](#题目2上传分片与断点续传)
- [题目3 · 线上 Bug 排查](#题目3线上-bug-排查)
...
> **考察要点**：阐述闭包本质、形成条件、内存机制；业务场景；内存泄漏与解决方案。
...
> **考察要点**：五种继承优缺点；Class 底层原理；原型链污染成因与防范。
...
> **考察要点**：浏览器与 Node 差异；宏任务/微任务顺序；结合 Promise、async/await、setTimeout、process.nextTick 分析输出。
...
> **考察要点**：A+ 规范；手写 then/catch/all/race 等；状态流转、值穿透、异常；allSettled/any 场景。
...
> **考察要点**：四种绑定规则；箭头函数 this；复杂场景判读；bind 原理与多次 bind。
...
> **考察要点**：标记清除、引用计数；常见泄漏场景；Chrome DevTools 定位与解决。
...
> **考察要点**：回调 / Promise / Generator / async-await 对比；async-await 与 Generator+Promise 原理。
...
> **考察要点**：变量/函数提升；var/let/const；TDZ；块级作用域。
...
> **考察要点**：HTTP/1~3 区别；HTTP/2 特性；HTTPS 加密与 TLS 握手。
...
### 题目7：Web 安全（XSS / CSRF 等）

> **考察要点**：XSS 分类与防护；CSRF；SQL 注入、点击劫持、MITM。
...
> **考察要点**：同源策略；iframe；postMessage；子域/主域数据共享。
...
### 题目9：Service Worker 与 PWA

> **考察要点**：原理与生命周期；离线缓存与推送；注册与注意事项。
...
> **考察要点**：核心差异；Composition vs Options；Proxy 与 defineProperty。
...
> **考察要点**：劫持、依赖收集、派发更新；computed/watch 原理与场景。
...
### 题目3：Vue 虚拟 DOM、diff 与指令

> **考察要点**：diff 细节与 Vue2/3 优化；key 与 index；v-for 与 v-if。
...
### 题目6：生命周期与 keep-alive

> **考察要点**：钩子流程；父子顺序；异步组件；keep-alive。
...
### 题目8：Teleport、Suspense 与编译优化

> **考察要点**：原理与场景；Vue3 编译优化。
...
> **考察要点**：为何引入 Fiber；调和流程；时间分片；与 React15 对比。
...
> **考察要点**：常用 Hooks 实现；链式规则；为何不能写在条件里。
...
> **考察要点**：区别；虚拟 DOM 与 diff；key 与列表优化。
...
### 题目4：useEffect / useLayoutEffect

> **考察要点**：依赖项；无限循环；执行时机与区别。
...
### 题目2：构建工具对比（Webpack / Vite 等）

> **考察要点**：差异；Vite 原理；相对 Webpack 优势。
...
### 题目3：Webpack 原理与 Loader/Plugin

> **考察要点**：原理；Loader vs Plugin；常用清单；手写示例；打包流程。
...
### 题目4：Tree-Shaking 与体积优化

> **考察要点**：原理与配置；Webpack/Vite 瘦身。
...
> **考察要点**：手写 debounce/throttle；立即执行与 cancel。
